// Metabolic
const C_INSTANCE_VAR = "metabolic";

// Static Accessor
const METABOLIC = {
    create: (context, dependencies) => {
        context[C_INSTANCE_VAR] = createInstance(dependencies);
        return context[C_INSTANCE_VAR];
    },
    getInstance: (callback) => {
        const instance = chrome.extension.getBackgroundPage()[C_INSTANCE_VAR];
        METABOLIC.chromex = chrome.extension.getBackgroundPage().ChromeEx(chrome);
        return instance;
    },
    pullTabsIn: function(tabIds, winId, callback) {
        METABOLIC.chromex.tabs.pullTabsIn(tabIds, winId, callback);
    },
    getCurrentWinId: function(chrome, callback) {
        METABOLIC.chromex.winId(callback);
    }
};

// Shared instance constructor
function createInstance(dependencies) {
    function Metabolic(dependencies) {
        this.dep = dependencies;
        this.init();
    }

    Object.assign(Metabolic.prototype, {
        anchorTabs: [],
        createAnchorTab: function(t) {
            // move tab to new window
            // tag window
            // pin tab
            // tag tab
            // pull in other tabs
            // route new tabs to window
        },
        classifyTabs: function(callback) {
            const me = this;
            const tree = {};

            me.tabs().done(metaTabs => {
                for (let id in metaTabs) {
                    const t = metaTabs[id];
                    if (t.tab.url)
                        addUrlToTree(tree, classifyUrl(new URL(t.tab.url)), t);
                }
            ...
            _generateMetaTab: function(tab) {
                ...
                return {
                    tab,
                    last_view: old_tab ? old_tab.last_view : null,
                    updated: now,
                    getUrl: function() { return new URL(this.tab.url) }
                };
            },
            ...
            removeTab: function(tabId) {
                var index = this._tabs.findIndex(function(tab) {
                    return tab.id == tabId;
                });
            ...
            loadTabLog: function() {
                ...
                //     me.log.tab_history = [].concat(items.trex_tab_log.filter(function(element) {
                //         return element
                //     }));
            ...
            _metaWinArray: function() {
                return $.map(this._meta_wins, function(meta_win, i) {
                    return [meta_win];
                });
            },
            ...
            pullTabsIn: function(tabIds, winId, callback) {
                chrome.windows.getCurrent(win => console.log(win.id + ` vs ${winId}`));
                this.dep.chromex.tabs.pullTabsIn(tabIds, winId, callback);
            },

            getCurrentWinId: function(callback) {
                this.dep.chromex.winId(callback);
            }
        }
    );

    return new Metabolic(dependencies);
}

function addUrlToTree(tree, urlHeir, tab) {
    let root = tree;

    while (urlHeir.length > 0) {
        const key = urlHeir.shift();
        root[key] = root[key] || {};
        root = root[key];
    }

    root["_tabs"] = root["_tabs"] || [];
    // root["_tabs"].push({
    //     name: tab.tab.url,
    //     id: tab.tab.id
    // });
    root["_tabs"].push(tab.tab);
...
function classifyUrl(url) {
    ...
    // Hostname starts with tld and works backwards
    let parts = url.hostname.split(".").reverse().map(p => "." + p);
    ...
    if (url.pathname)
        parts = parts.concat(url.pathname.split("/").map(p => "/" + p));
