function ChromeEx(chrome) {
	return {
		winId: (callback) => {
			chrome.windows.getCurrent((win) => callback(win.id));
		},
		util: {
			isArray: (value) => (value && typeof value === 'object') ? toString.call(value) === '[object Array]' : false
		},
		tabs: {
			...
			pullTabsIn: function (tabIds, winId, callback) {
				if (typeof(winId) === 'number') {
					console.log(`chrome.tabs.move(${tabIds}, {windowId: ${winId}, index: -1}, ${callback});`);
					chrome.tabs.move(tabIds, {windowId: winId, index: -1}, callback);
				}
				else if (typeof(winId) === 'object' && typeof(winId.id) === 'number') {
					chromex.tabs.pullTabsIn(tabIds, winId.id, callback);
				}
				else {
					chrome.windows.getCurrent(win => {
						if (win !== null && typeof(win.id) === 'number')
							chromex.tabs.pullTabsIn(tabIds, win.id, callback);
					});
				}
			},
